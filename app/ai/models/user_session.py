...
class UserSession:
    ...
    def get_messages_for_ai(self) -> List[ChatMessage]:
        """獲取適合 AI 處理的訊息格式"""
        return self.history
    ...
    def get_last_user_message(self) -> Optional[ChatMessage]:
        """獲取最後一條用戶訊息"""
        return self._get_last_message_by_role("user")

    def get_last_assistant_message(self) -> Optional[ChatMessage]:
        """獲取最後一條助手訊息"""
        return self._get_last_message_by_role("assistant")
    ...
    def get_conversation_context_summary(self) -> dict:
        """獲取對話上下文摘要（調試用）"""
        return {
            "is_fresh": self.is_fresh_conversation(),
            "user_messages_count": len(self.get_messages_by_role("user")),
            "assistant_messages_count": len(self.get_messages_by_role("assistant")),
            "has_search_criteria": bool(self.data),
            "conversation_type": (
                "fresh" if self.is_fresh_conversation() else "continuing"
            ),
        }
    ...
    def _get_last_message_by_role(self, role: str) -> Optional[ChatMessage]:
        """獲取指定角色的最後一條訊息"""
    ...
    def __str__(self) -> str:
        return f"UserSession(user_id={self.user_id}, messages={len(self.history)})"

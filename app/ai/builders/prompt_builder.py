...
class SystemPromptBuilder:
    ...
    # === Convenience Methods - 類似 Android 的 helper methods ===
    def get_legacy_prompt(self) -> str:
        """獲取原有的餐廳搜尋 Prompt（向後兼容）"""
        return self.build_prompt(PromptType.LEGACY_RESTAURANT_SEARCH)

    def get_intent_analysis_prompt(self, **kwargs) -> str:
        """獲取意圖分析 Prompt"""
        return self.build_prompt(PromptType.INTENT_ANALYSIS, **kwargs)

    def get_follow_up_prompt(self, **kwargs) -> str:
        """獲取後續問題 Prompt"""
        return self.build_prompt(PromptType.FOLLOW_UP_QUESTION, **kwargs)

    def get_search_response_prompt(self, **kwargs) -> str:
        """獲取搜尋回應 Prompt"""
        return self.build_prompt(PromptType.SEARCH_RESPONSE, **kwargs)

    def get_general_chat_prompt(self, **kwargs) -> str:
        """獲取一般對話 Prompt"""
        return self.build_prompt(PromptType.GENERAL_CHAT, **kwargs)
